...
- Follow-up P1 resolved: `VietnameseFallback.asset` now has a real Noto Sans source, populated glyph/character tables for U+1EA0-U+1EF9, U+0110/U+0111, and U+01A0-U+01B0, and persisted atlas/material sub-assets while retaining Dynamic fallback mode. Focused Unity `UIThemeTests` passed 3/3 in `/tmp/task3-font-final.xml` with Unity 6000.3.23f1. The temporary generator was removed; `NotoSansVietnamese.ttf` is the checked-in source asset.
