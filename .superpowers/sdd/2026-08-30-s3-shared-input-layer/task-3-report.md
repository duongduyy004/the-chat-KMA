...
- Full PlayMode: `rtk ~/.local/bin/unity test . --mode PlayMode --output /tmp/kma-s3-final-playmode.xml --timeout 600 -- -nographics` — interrupted on request while repeatedly loading scenes after the headless TMP importer reported `No graphic device is available` for `MG_retired-route`; no complete result count was produced.
