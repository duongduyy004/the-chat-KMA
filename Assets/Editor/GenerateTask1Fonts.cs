...
internal static class GenerateTask1Fonts
{
    ...
    public static void Run()
    {
        ...
        var baloo = Create("Baloo2-ExtraBold", characters);
        var nunito = Create("Nunito-Bold", characters);
        var fallback = Create("VietnameseFallback", RequiredCharacters(), "Baloo2-ExtraBold");
    ...
    private static TMP_FontAsset Create(string assetName, string characters)
    {
    ...
    private static TMP_FontAsset Create(string assetName, string characters, string sourceAssetName)
    {
        ...
        fontAsset.name = assetName;
        if (!fontAsset.TryAddCharacters(characters, out var missingCharacters))
            throw new InvalidDataException($"{assetName} missing required characters: {missingCharacters}");
        fontAsset.atlasPopulationMode = AtlasPopulationMode.Static;
        AssetDatabase.CreateAsset(fontAsset, assetPath);
        return fontAsset;
    }
